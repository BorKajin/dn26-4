...
mid_a = movies.loc[movies["title"] == film_a, "movieId"].iloc[0]
mid_b = movies.loc[movies["title"] == film_b, "movieId"].iloc[0]

ratings_a = ratings[ratings["movieId"] == mid_a]
ratings_b = ratings[ratings["movieId"] == mid_b]

ha = ratings_a["rating"].value_counts().reindex([1,2,3,4,5], fill_value=0).reset_index()
ha.columns = ["rating", "count"]
hchart_a = alt.Chart(ha).mark_bar().encode(x=alt.X('rating:O', title='Ocena'), y=alt.Y('count:Q', title='Število'))
st.subheader(film_a)
st.altair_chart(hchart_a, use_container_width=True)

hb = ratings_b["rating"].value_counts().reindex([1,2,3,4,5], fill_value=0).reset_index()
hb.columns = ["rating", "count"]
hchart_b = alt.Chart(hb).mark_bar(color="orange").encode(x=alt.X('rating:O', title='Ocena'), y=alt.Y('count:Q', title='Število'))
st.subheader(film_b)
st.altair_chart(hchart_b, use_container_width=True)

st.markdown("## Povprečna ocena po letih")
ya = ratings_a.groupby("year")["rating"].mean().reset_index(name="avg")
yb = ratings_b.groupby("year")["rating"].mean().reset_index(name="avg")
